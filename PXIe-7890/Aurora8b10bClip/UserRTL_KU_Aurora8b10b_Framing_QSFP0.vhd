-------------------------------------------------------------------------------
--
-- File: UserRTL_KU_Aurora8b10b_Framing_QSFP0.vhd
...
entity UserRTL_KU_Aurora8b10b_Framing_QSFP0 is
  port (
    ------------------------------
    -- Reset                    --
    ------------------------------
...
end UserRTL_KU_Aurora8b10b_Framing_QSFP0;

architecture rtl of UserRTL_KU_Aurora8b10b_Framing_QSFP0 is
  ...
  -- CLIP signals                                            --
  -------------------------------------------------------------
  signal MgtRefclk  : std_logic_vector(Qsfp0MgtRefClk_p'range);
...
begin
  ...
  IbufdsGen: for i in Qsfp0MgtRefClk_p'range generate

    --vhook_i IBUFDS_GTE3 IBUFDS_GTE3_inst0 hidegeneric=true
    --vhook_a I      Qsfp0MgtRefClk_p(i)
    --vhook_a IB     Qsfp0MgtRefClk_n(i)
    --vhook_a CEB    '0'
    ...
    IBUFDS_GTE3_inst0: IBUFDS_GTE3
      port map (
        ...
        CEB   => '0',                  --in  std_ulogic
        I     => Qsfp0MgtRefClk_p(i),  --in  std_ulogic
        IB    => Qsfp0MgtRefClk_n(i)); --in  std_ulogic

  end generate;

  -- On-board 80 MHz clock for use with CLIP interfacing signals
  InitClk <= Qsfp0SocketClk80;
  ...
  -- MGT Port
  -----------------------------------------------------------------------------
  PortRx_p    (0 to 3)     <= reversi(Qsfp0MgtRx_p(3 downto 0));
  PortRx_n    (0 to 3)     <= reversi(Qsfp0MgtRx_n(3 downto 0));
  Qsfp0MgtTx_p(3 downto 0) <= reversi(PortTx_p   (0 to 3));
  Qsfp0MgtTx_n(3 downto 0) <= reversi(PortTx_n   (0 to 3));
