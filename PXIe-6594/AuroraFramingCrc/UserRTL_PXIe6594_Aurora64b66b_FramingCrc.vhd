-------------------------------------------------------------------------------
--
-- File: UserRTL_PXIe6594_Aurora64b66b_FramingCrc.vhd
...
--
-- Purpose:
--   This CLIP instantiates an Aurora 64b66b core for the PXIe-6594. This core
...
entity UserRTL_PXIe6594_Aurora64b66b_FramingCrc is
  port (
    ------------------------------
    -- Reset                    --
    ------------------------------
    ...
    -- These clocks run at the line rate/64. By default, at 25.0Gbps, these
    -- clocks run at 390.63MHz.
    ...
    cPort0_mmcm_not_lock_out    : out std_logic;  -- Driven from userclk active, user guide says init_clk but it is wrong
    uPort0_crc_pass_fail_n      : out std_logic;
    uPort0_crc_valid            : out std_logic;
    ...
    cPort1_mmcm_not_lock_out    : out std_logic;  -- Driven from userclk active, user guide says init_clk but it is wrong
    uPort1_crc_pass_fail_n      : out std_logic;
    uPort1_crc_valid            : out std_logic;
...
end UserRTL_PXIe6594_Aurora64b66b_FramingCrc;

architecture rtl of UserRTL_PXIe6594_Aurora64b66b_FramingCrc is
  ...
  constant kNumPorts : integer := 2;  -- number of ports on front panel
  ...
  --vhook_d aurora_64b66b_DF_crc
  component aurora_64b66b_DF_crc
    port (
  ...
  -- CLIP signals                                            --
  -------------------------------------------------------------
  signal FAM_refclk  : std_logic_vector(3 downto 0);
  ...
  signal gtwiz_tTxDiffCtrl_in   : std_logic_vector(kNumPorts*kNumLanes*5-1  downto 0);
  ...
  -- signal aGtTxPmaResetDone   : std_logic_vector(kNumPorts*kNumLanes-1 downto 0);
  signal aTxPcsReset_in      : std_logic_vector(kNumPorts*kNumLanes-1 downto 0);
  ...
  signal sDMonitorOut_out   : GTYE4DMonitorOutAry_t(kNumPorts*kNumLanes-1 downto 0);

  signal rRxRate_in      : GTRateSelAry_t  (kNumPorts*kNumLanes-1 downto 0);

  signal tTxDiffCtrl_in   : GTYDiffCtrlAry_t (kNumPorts*kNumLanes-1 downto 0);
  signal aTxPreCursor_in  : GTYCursorSelAry_t(kNumPorts*kNumLanes-1 downto 0);
  signal aTxPostCursor_in : GTYCursorSelAry_t(kNumPorts*kNumLanes-1 downto 0);
  ...
  signal Port_crc_pass_fail_n, Port_crc_valid : std_logic_vector(kNumPorts-1 downto 0);
...
begin
  ...
  uPort0_crc_pass_fail_n <= Port_crc_pass_fail_n(0);
  uPort1_crc_pass_fail_n <= Port_crc_pass_fail_n(1);

  uPort0_crc_valid <= Port_crc_valid(0);
  uPort1_crc_valid <= Port_crc_valid(1);
  ...
  GenSigAssignment : for i in 0 to kNumPorts*kNumLanes-1 generate
    ...
    gtwiz_tTxDiffCtrl_in  ((i+1)*5-1 downto i*5) <= tTxDiffCtrl_in  (i);
  ...
  gtwiz_slv_dmonclk  <= s_aclk & s_aclk & s_aclk & s_aclk & s_aclk & s_aclk & s_aclk & s_aclk;

  GenGtwizDMonClk : for i in 0 to kNumPorts*kNumLanes-1 generate
    sDMonitorOut_out(i) <= gtwiz_dDMonitorOut_out((i+1)*16-1 downto i*16);
  ...
  begin
    GenAurora : for i in 0 to kNumPorts-1 generate
      ...
      --vhook_a MgtRefClk            FAM_refclk(0)
      ...
      --vhook_a aTxPcsReset_in       aTxPcsReset_in  ((i+1)*kNumLanes-1 downto i*kNumLanes)
      --vhook_a aEyeScanReset_in     aEyeScanReset_in((i+1)*kNumLanes-1 downto i*kNumLanes)
      --vhook_a aGtPowerGood_out     aGtPowerGood_out((i+1)*kNumLanes-1 downto i*kNumLanes)
      ...
      AXI4Lite_GTYE4_Control_Regs4_inst: AXI4Lite_GTYE4_Control_Regs4
        port map (
          ...
          MgtRefClk                   => FAM_refclk(0),                                             --in  std_logic
          ...
          aTxPcsReset_in              => aTxPcsReset_in ((i+1)*kNumLanes-1 downto i*kNumLanes),     --out std_logic_vector(4-1:0)
          aEyeScanReset_in            => aEyeScanReset_in((i+1)*kNumLanes-1 downto i*kNumLanes),    --out std_logic_vector(4-1:0)
          aGtPowerGood_out            => aGtPowerGood_out((i+1)*kNumLanes-1 downto i*kNumLanes),    --in  std_logic_vector(4-1:0)
          ...
          dDMonitorOut_out            => sDMonitorOut_out((i+1)*kNumLanes-1 downto i*kNumLanes),    --in  GTYE4DMonitorOutAry_t(4-1:0)
          ...
          tTxDiffCtrl_in              => tTxDiffCtrl_in ((i+1)*kNumLanes-1 downto i*kNumLanes),     --out GTYDiffCtrlAry_t(4-1:0)
          aTxPreCursor_in             => aTxPreCursor_in ((i+1)*kNumLanes-1 downto i*kNumLanes),    --out GTYCursorSelAry_t(4-1:0)
          aTxPostCursor_in            => aTxPostCursor_in((i+1)*kNumLanes-1 downto i*kNumLanes),    --out GTYCursorSelAry_t(4-1:0)
      ...
      --vhook aurora_64b66b_DF_crc Aurora_PortN
      ...
      --vhook_a crc_pass_fail_n  Port_crc_pass_fail_n(i)
      --vhook_a crc_valid        Port_crc_valid (i)
      ...
      --vhook_a gt_txdiffctrl        gtwiz_tTxDiffCtrl_in  ((i+1)*kNumLanes*5-1 downto i*kNumLanes*5)
      ...
      --vhook_a gt_txpcsreset        aTxPcsReset_in((i+1)*kNumLanes-1 downto i*kNumLanes)
      ...
      --vhook_a gt_dmonitorout       gtwiz_dDMonitorOut_out((i+1)*kNumLanes*16-1 downto i*kNumLanes*16)
      --vhook_a gt_cplllock          aCpllLock_out ((i+1)*kNumLanes-1 downto i*kNumLanes)
      --vhook_a gt_qplllock          open
      Aurora_PortN: aurora_64b66b_DF_crc
        port map (
          ...
          crc_pass_fail_n             => Port_crc_pass_fail_n(i),                                             --out STD_LOGIC
          crc_valid                   => Port_crc_valid (i),                                                  --out STD_LOGIC
          ...
          gt0_drpaddr                 => gtwiz_drpaddr_in((i*4+1)*kAddrSize-1 downto (i*4+0)*kAddrSize),      --in  STD_LOGIC_VECTOR(9:0)
          ...
          gt1_drpaddr                 => gtwiz_drpaddr_in((i*4+2)*kAddrSize-1 downto (i*4+1)*kAddrSize),      --in  STD_LOGIC_VECTOR(9:0)
          ...
          gt2_drpaddr                 => gtwiz_drpaddr_in((i*4+3)*kAddrSize-1 downto (i*4+2)*kAddrSize),      --in  STD_LOGIC_VECTOR(9:0)
          ...
          gt3_drpaddr                 => gtwiz_drpaddr_in((i*4+4)*kAddrSize-1 downto (i*4+3)*kAddrSize),      --in  STD_LOGIC_VECTOR(9:0)
          ...
          gt_txdiffctrl               => gtwiz_tTxDiffCtrl_in ((i+1)*kNumLanes*5-1 downto i*kNumLanes*5),     --in  STD_LOGIC_VECTOR(19:0)
          ...
          gt_txpcsreset               => aTxPcsReset_in((i+1)*kNumLanes-1 downto i*kNumLanes),                --in  STD_LOGIC_VECTOR(3:0)
          ...
          gt_dmonitorout              => gtwiz_dDMonitorOut_out((i+1)*kNumLanes*16-1 downto i*kNumLanes*16),  --out STD_LOGIC_VECTOR(63:0)
